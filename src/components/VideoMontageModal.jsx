...
export default function VideoMontageModal({ videos, auth, folderId, folderName }) {
  const { closeWizard, prepareExport, job } = useVideoMontage()
  const onClose = closeWizard
  const [stage, setStage] = useState(job ? 2 : 0) // 0=select, 1=edit, 2=render
    ...
    <div className="vmm-overlay" onClick={e => { if (e.target.classList.contains('vmm-overlay')) onClose() }}>
      <div className="vmm-modal">
        ...
        <div className="vmm-footer">
          ...
          {stage === 1 && (
            <button
              onClick={() => { prepareExport(clips, folderId, folderName); setStage(2) }}
          ...
          {stage === 2 && job && (
            <button
              onClick={onClose}
              style={{ padding: '7px 18px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: 13 }}
            >
              Minimizza
            </button>
          )}
